#Introduction: request of user name and explanation of the game
...
#IMPORTANT!!! The range will stop before the second numerb inside the parentesis)
...
while user_num != sec_num and count <=(5):
    ...
    #This function add an excepiont in case the user do not insert a number
    #if is a number give me a True value, otherwhise instead of giving error message return an True value
    def check(user_num):
        try:
            int(user_num)
            return True
            ...
            return False
    #If the value of the function we ask again to insert a number    
    ...
    if int(user_num) < sec_num:
        #here i check if the difference between the secret number and the user number is lower or equal than 5
        if sec_num - user_num <= 5:
            print ("You are very close! GO GO GO!")
            print ("Secret number is higher!"+'\n')
        #here i check if the difference between the secret number and the user number is higher or equal than 40
